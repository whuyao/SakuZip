...
int32_t mz_zip_reader_entry_save(void *handle, void *stream, mz_stream_write_cb write_cb) {
    ...
    if (reader->progress_cb) {
        err_cb = reader->progress_cb(reader, reader->progress_userdata, reader->file_info, current_pos);
        if (err_cb != MZ_OK)
            return err_cb;
    }
...
int32_t mz_zip_reader_entry_save_file(void *handle, const char *path) {
    ...
    if (reader->entry_cb) {
        err_cb = reader->entry_cb(reader, reader->entry_userdata, reader->file_info, pathwfs);
        if (err_cb != MZ_OK) {
            err = err_cb;
            goto save_cleanup;
        }
...
int32_t mz_zip_writer_entry_open(void *handle, mz_zip_file *file_info) {
    ...
    if (writer->entry_cb) {
        err_cb = writer->entry_cb(writer, writer->entry_userdata, &writer->file_info);
        if (err_cb != MZ_OK)
            return err_cb;
    }
...
int32_t mz_zip_writer_add(void *handle, void *stream, mz_stream_read_cb read_cb) {
    ...
    if (writer->progress_cb) {
        err_cb = writer->progress_cb(writer, writer->progress_userdata, &writer->file_info, current_pos);
        if (err_cb != MZ_OK)
            return err_cb;
    }
