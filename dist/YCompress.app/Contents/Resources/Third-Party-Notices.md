...
YCompress includes selected source files from
...
YCompress carries a small local change in `mz_zip_rw.c` so a non-zero return
from entry/progress callbacks immediately stops an operation. This connects
the library to YCompress pause and cancellation controls.

minizip-ng is distributed under the zlib license. The complete upstream
license is retained at:

`Sources/CYCompressArchive/minizip/LICENSE`
